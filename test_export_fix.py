...
Ces idées représentent les solutions les plus prometteuses."""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            save_full_log(test_objectif, test_contexte, test_contraintes, test_logs, test_synthese, None)
            print(f"✅ save_full_log terminé sans erreur")
            ...
            if logs_path.exists():
                log_files = list(logs_path.glob("*.yaml")) + list(logs_path.glob("*.json")) + list(logs_path.glob("*.md"))
                print(f"📋 Fichiers de log générés: {len(log_files)}")
                for f in log_files:
                    print(f"   • {f.name} ({f.stat().st_size} bytes)")
            else:
                print(f"❌ Répertoire logs non trouvé")
            ...
            if exports_path.exists():
                ...
                for f in export_files:
                    ...
                    if size > 50:  # Si plus de 50 bytes, probablement un contenu détaillé
                        print(f"     ✅ Contenu détaillé (> 50 bytes)")
                        ...
                        try:
                            with open(f, "r", encoding="utf-8") as file:
                                ...
                                print(f"     📄 Aperçu: {lines[0][:60]}..." if lines else "     📄 Fichier vide")
                                
                                # Vérifier la présence des sections attendues
                                sections = ["Plan Initial", "Critique du Plan", "Défense du Plan", "Plan Final Révisé"]
                                found_sections = [s for s in sections if s in content]
                                print(f"     📋 Sections trouvées: {len(found_sections)}/{len(sections)}")
                        except Exception as e:
                            print(f"     ❌ Erreur lecture: {e}")
                    else:
                        print(f"     ⚠️ Contenu probablement trop court")
            else:
                print(f"❌ Répertoire exports non trouvé")
